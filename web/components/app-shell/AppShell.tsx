...
function resolveAppshellDetailTone(status: string | undefined): keyof typeof appshelldetailTone {
  if (status && status in appshelldetailTone) return status as keyof typeof appshelldetailTone;
  return 'queued';
}
...
function groupAppshellBrowserByType<T extends { type?: string }>(items: T[]): Map<string, T[]> {
...
function countAppshellBrowserByStatus<T extends { status?: string }>(items: T[]): Record<string, number> {
